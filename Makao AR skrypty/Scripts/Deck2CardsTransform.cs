...
using TMPro;
public class Deck2CardsTransform : MonoBehaviour
{
    // Animator _animator;
    ...
    #region Zamiana kart       
        ...
        if (Deck3.transform.GetChild(6).GetComponent<BoxCollider>().enabled == true)
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                ...
                HelpState.text = "Kolejka gracza 3";
            }
            Deck2.GetComponent<Deck2CardsTransform>().enabled = false;
            ...
            case "Wojenna":
                switch (SelectedCardSymbol)
                {
                    case "2":
                        ...
                        for (int j = 0; j < 2; j++)
                        {
                            int ilosc_slotow = Panel3.transform.childCount;
                            for (int i = 0; i < ilosc_slotow; i++)
                            {
                                Transform slot = Panel3.transform.GetChild(i);
    ...
    #region Zamiana kart       
        ...
        if (Deck3.transform.GetChild(6).GetComponent<BoxCollider>().enabled == true)
        {
            ...
            {
                Deck2.transform.GetChild(i).GetComponent<BoxCollider>().enabled = false;
                HelpState.text = "Zmieniono kolor na " + mid_card_child.GetComponent<CardValues>().color + ", kolejka gracza 3";
            }
            Deck2.GetComponent<Deck2CardsTransform>().enabled = false;
            Deck2.GetComponent<CheckCards2>().enabled = false;            
            Deck3.GetComponent<CheckCards3>().enabled = true;            
    ...
    #region Zamiana kart            
        ...
        if (Deck4.transform.GetChild(6).GetComponent<BoxCollider>().enabled == true)
        {
            ...
            {
                Deck2.transform.GetChild(i).GetComponent<BoxCollider>().enabled = false;
                HelpState.text = "Kolejka gracza 4, gracz 3 zostaje pominięty";
            }
            Deck2.GetComponent<Deck2CardsTransform>().enabled = false;
            Deck2.GetComponent<CheckCards2>().enabled = false; 
...
void Update()
    {
        if (Input.touchCount == 1 && Input.touches[0].phase == TouchPhase.Ended)
        {
            ...
            if (Physics.Raycast(raycast, out raycastHit))
            {
                ...
                for (int i=0; i<7;i++){
                    Deck2.transform.GetChild(i).GetComponent<GetDeckCardValue>().enabled = true;
                }
