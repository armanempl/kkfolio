...
using TMPro;
public class Deck3CardsTransform : MonoBehaviour
{
    // Animator _animator;
    ...
    #region Zamiana kart       
        ...
        #region Nastepny gracz
            for (int j = 0; j < 7; j++)
            {
                Deck4.transform.GetChild(j).GetComponent<BoxCollider>().enabled = true;
            }
        if (Deck4.transform.GetChild(6).GetComponent<BoxCollider>().enabled == true)
        {
            Deck4.GetComponent<Deck4CardsTransform>().enabled = true;
            for (int i = 0; i < 7; i++)
            {
                Deck3.transform.GetChild(i).GetComponent<BoxCollider>().enabled = false;
                HelpState.text = "Kolejka gracza 4";
            }
            Deck3.GetComponent<Deck3CardsTransform>().enabled = false;
            Deck3.GetComponent<CheckCards3>().enabled = false; 
            Deck4.GetComponent<CheckCards4>().enabled = true;             
        } 
    #endregion
    #endregion                 
            ...
            case "Wojenna":
                switch (SelectedCardSymbol)
                {
                     case "2":
                        ...
                        for (int j = 0; j < 2; j++)
                        {
                            int ilosc_slotow = Panel4.transform.childCount;
                            for (int i = 0; i < ilosc_slotow; i++)
                            {
                                Transform slot = Panel4.transform.GetChild(i);
    ...
    #region Zamiana kart       
        ...
        if (Deck4.transform.GetChild(6).GetComponent<BoxCollider>().enabled == true)
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                Deck2.transform.GetChild(i).GetComponent<BoxCollider>().enabled = false;
                HelpState.text = "Zmieniono kolor na " + mid_card_child.GetComponent<CardValues>().color + ", kolejka gracza 4";
            }
            Deck3.GetComponent<Deck3CardsTransform>().enabled = false;
            Deck3.GetComponent<CheckCards3>().enabled = false; 
            Deck4.GetComponent<CheckCards4>().enabled = true;             
    ...
    #region Zamiana kart      
        ...
        //Debug.Log(stopQueue);
            ...
            case "Postojowa":
                stopQueue = true;
                HelpState.text = "Następny gracz pomija kolejkę";
    #region Zamiana kart            
                old_mid_card = mid_card.transform.GetChild(0);
                old_mid_card.SetParent(CardStash, false);
                old_mid_card.transform.localPosition = new Vector3(-2, -1, -2);
        ...
        #region Nastepny gracz
            for (int j = 0; j < 7; j++)
            {
                Deck1.transform.GetChild(j).GetComponent<BoxCollider>().enabled = true;
            }
        if (Deck1.transform.GetChild(6).GetComponent<BoxCollider>().enabled == true)
        {
            Deck1.GetComponent<DeckCardsTransform>().enabled = true;
            for (int i = 0; i < 7; i++)
            {
                Deck3.transform.GetChild(i).GetComponent<BoxCollider>().enabled = false;
                HelpState.text = "Kolejka gracza 1, gracz 4 zostaje pominięty";
            }
            Deck3.GetComponent<Deck3CardsTransform>().enabled = false; 
            Deck3.GetComponent<CheckCards3>().enabled = false; 
            //Deck4.GetComponent<CheckCards4>().enabled = true;                             
    ...
    #endregion
            case "Zwykła":
                HelpState.text = "Wyrzucono zwykłą kartę";
    #region Zamiana kart            
                old_mid_card = mid_card.transform.GetChild(0);
                old_mid_card.SetParent(CardStash, false);
                old_mid_card.transform.localPosition = new Vector3(-2, -1, -2);
        ...
        if (Deck4.transform.GetChild(6).GetComponent<BoxCollider>().enabled == true)
        {
            ...
            Deck4.GetComponent<CheckCards4>().enabled = true; 
            Deck3.GetComponent<CheckCards3>().enabled = false;            
            //Deck2.GetComponent<CheckCards2>().enabled = false;
        } 
    #endregion
    #endregion                                    
...
void Update()
    {
        if (Input.touchCount == 1 && Input.touches[0].phase == TouchPhase.Ended)
        {
            ...
            if (Physics.Raycast(raycast, out raycastHit))
            {
                ...
                for (int i=0; i<7;i++){
                    Deck3.transform.GetChild(i).GetComponent<GetDeckCardValue>().enabled = true;
                }
